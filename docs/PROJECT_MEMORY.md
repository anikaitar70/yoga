...
## Universal Section Backgrounds (2026-08-29) — Fixed 2026-08-30

- **System:** `SectionBackground` (`src/components/content/SectionBackground.tsx`) — `resolveSectionBackgroundStyle()` produces inline `backgroundColor/backgroundImage` that is authoritative over legacy class-based backgrounds. Wrapper in `PageSectionRenderer.tsx:54` + direct inline on `ProgramSectionShell`'s `<section>` element (the visual owner). `AboutSectionShell` also applies same style via `layout` prop.
- **Modes:** `auto` (preserve legacy `styleClass` warm/muted/immersive + per-block defaults like `bg-primary-soft/15`), `none` (`backgroundColor: transparent; backgroundImage: none; boxShadow: none` → page background shows), `solid` (`backgroundColor: #hex; backgroundImage: none`), `image` (`backgroundImage: url(...); cover/center/no-repeat`). All modes survive hydration, scroll, resize, locale change.
- **Config:** Stored in `PageSectionRecord.layout.sectionBackground` (`SectionBackgroundSettings` in `src/lib/section-layout.ts:49`). `textContainerSchema` validates `mode/color/imageUrl`. Default `auto` when missing → backward compatible, existing pages unchanged.
- **Authoritative rendering:** Prior bug: `PageSectionRenderer` applied solid green to outer `<div class="w-full">` only. Inner `ProgramSectionShell`'s `<section>` had legacy class backgrounds (`bg-primary-soft/15`, `bg-accent-soft/20`, `bg-surface-warm/30`, gradients, `program-rhythm-*` glows). Before `MotionReveal` mounted, `<section>` opacity 0 → outer green visible. After `MotionReveal` (`framer-motion` rise/fade, 0.9s) opacity→1, inner default background covered outer green and card square returned. Fix: `ProgramSectionShell.tsx:73` now computes `sectionBgStyle = resolveSectionBackgroundStyle(layout.sectionBackground)` and merges it into `<section style={{...sectionPaddingStyle, ...textStyleCss, ...sectionBgStyle}}>`; when `mode !== "auto"` it suppresses legacy `styleClass`/`theme.sectionRhythm` and inline `backgroundImage:none` clears gradients. Animation now animates transform/opacity only, not background; after completion inline style remains (`#1ef15d` stays). Hero (`ProgramHeroBlock` → `ProgramSectionShell` with `theme.heroClass`) and all `PageSectionType` blocks (`HERO`, `IMAGE_TEXT`, `GALLERY`, `TESTIMONIALS`, `EVENTS`, `CONTACT`, `CUSTOM_TEXT`, `DYNAMIC_IMAGE_TEXT`) go through this path; About pages via `AboutSectionShell` with same style.
- **Section vs TextContainer:** Independent. `SectionBackground` controls whole section/container; `TextContainer` (`src/components/content/TextContainer.tsx`) controls inner text card (`mode auto/none/solid/image` with own `resolveTextContainerStyle`). Example: section solid green + text none → green section, transparent text; section none + text solid white → transparent section, white card. `ProgramParagraphGrid` cards respect `textContainer` only, not section background.
- **Admin:** `PageSectionsManager.tsx` and `EventPageSectionsManager.tsx` `LayoutEditor` (shared) exposes `Section background` select (Auto/None/Solid/Image) + color picker + `ImageUploadField section="pages"` for image; same for `Text background` separate control. `HERO` now shows same control (generic LayoutEditor), special-event `/admin/special-events/[id]` reuses same `LayoutEditor`.
- **All section types supported:** HERO, IMAGE_TEXT, GALLERY, TESTIMONIALS, EVENTS, CONTACT, CUSTOM_TEXT, DYNAMIC_IMAGE_TEXT via `PageSectionRenderer` + `ProgramSectionShell`/`AboutSectionShell`; Special Event sections via `SpecialEventSections.tsx → PageSectionRenderer`. Home-page `HeroSectionView` is separate model (`HeroSection` table) and not yet migrated to sectionBackground (documented limitation).
- **Animation interaction:** `MotionReveal` (`src/components/program/MotionReveal.tsx`) uses `whileInView` opacity/y only; no longer changes background classes on completion. `prefers-reduced-motion` disables animation entirely (plain `<section>`).
- **Backward compat:** No DB migration; missing `sectionBackground` → `auto`.
- **Testing:** `npm run lint` 81 errors/59 warnings (baseline), `npm run build` compiled successfully (40/40 pages). Browser/DB smoke testing NOT PERFORMED because `DATABASE_URL` dev database unavailable — static code-level verification of complete pipeline (admin save → `layout` JSON → `resolveSectionBackgroundStyle` → `ProgramSectionShell` inline → remains after motion) only.

## Testimonials Navigation (2026-08-29)

- **Config:** `Testimonials` included in `SECONDARY_NAV_HREFS` in `lib/site-navigation.ts`.
- **Rendering:** `SiteHeader.tsx` maps `secondaryNav` to hamburger/More menu.
- **Routes:** `/testimonials` (EN), `/ja/testimonials` (JA).
- **Testing:** Build verified. Browser testing NOT PERFORMED.

## Dynamic Image + Text Section (2026-08-29)

- **Section type:** `DYNAMIC_IMAGE_TEXT` — `PageSectionType` enum (Prisma + `page-section-types.ts`) with label "Dynamic Image + Text". Separate from `IMAGE_TEXT`; existing section untouched.
- **Purpose:** Multiple image/text pairs in one section with optional sticky-image scrolling. Desktop: image sticky while its associated text scrolls; mobile: stacks `Image 1 / Text 1, Image 2 / Text 2`. No scroll hijacking — normal browser scroll + CSS `position: sticky`.
- **Storage:** `PageSection.payload` JSON (no new table). Shape `DynamicImageTextPayload` (`src/lib/page-section-types.ts:150`):
  ```ts
  { scrollBehavior: "normal"|"sticky", layoutDirection: "image-left"|"image-right", imageHeight: "auto"|"small"|"medium"|"large", imageFit: "cover"|"contain", items: { id, imageUrl, imageAlt?, content: string (EN HTML), contentJa?: string (JA HTML) }[] }
  ```
  Defaults sticky/image-left/medium/cover, `items: []`. Validated by `dynamicImageTextPayloadSchema` / `sanitizeDynamicImageTextPayload` (`src/lib/page-section-payloads.ts`), sanitized on save via `sanitizeDynamicImageTextPayload` (`src/lib/rich-text-server.ts`) for `content`/`contentJa` HTML allowlist.
- **Layout:** Section-level `layout` holds universal `sectionBackground` (`auto|none|solid|image`) and `textContainer` plus spacing/contentWidth/textAlignment/imageAspect etc. (`defaultLayoutForSectionType("DYNAMIC_IMAGE_TEXT")` in `src/lib/section-layout.ts`). Preview controls mapped in `src/lib/preview-layout-controls.ts`. `SectionBackground` wrapper in `PageSectionRenderer.tsx:52` applies section background independent of content images.
- **Renderer:** `src/components/content/sections/DynamicImageTextSection.tsx` — async Server Component. Reads locale via `getLocale()` to pick `contentJa` vs `content`. Per-item grid `lg:grid-cols-2 lg:items-start` with image column `lg:sticky lg:top-24 self-start` when `scrollBehavior==="sticky"` else static. `imageHeight` maps to 240/360/500px (auto -> `aspect-[4/3]`). `imageFit` cover/contain via `object-cover`/`object-contain`. Images via `next/image` with `unoptimized={isLocalUploadUrl}`. Text wrapped in `TextContainer settings={section.layout?.textContainer}` + `RichText html={safeHtml}` (server-sanitized). Section title/subtitle via `SectionBrandTitle`. Each item row's sticky container is the row itself, so sticky ends at section boundary (correct exit, no overlap onto next section). Mobile media query defaults to stacked (no sticky).
- **Scroll behavior (chosen impl):** Option A — multiple image/text rows with per-row `position: sticky`. Alternative B (single sticky viewport + JS swapping) rejected: requires IntersectionObserver, more complexity, violates "prefer CSS sticky, no scroll listeners". A's sticky is bounded by row height: long text sticks image while text scrolls, short text shows no empty gap, final section exits together. No JS scroll listeners, no animation library, respects `prefers-reduced-motion` (no transitions to disable).
- **Admin editor:** `src/components/admin/PageSectionsManager.tsx:1139` — `PayloadEditor` branch for `DYNAMIC_IMAGE_TEXT`. Controls: scroll behavior (Sticky/Normal), layout direction, image height, image fit, items list with `+ Add item`, per-item `ImageUploadField section="pages"`, imageAlt input, `RichTextEditor` for English and Japanese (each item), reorder ↑/↓, Remove. Normalizes/payload on save (`normalizePayloadForSave`) filters items requiring `imageUrl` + `content`. Also supports Event Page Sections via same `PAGE_SECTION_TYPES` + defaults; event routes sanitize same payload shape. Uses existing `ImageUploadField`/`RichTextEditor` — no duplicate upload system.
- **Localization:** Images shared (single `imageUrl`); per-item `content` (EN) / `contentJa` (JA). JA shown when `x-nirvana-locale: ja` (via proxy), fallback to EN if blank. No `SiteConfig.localeContent` array merge needed for items (direct payload fields).
- **Validation/sanitization:** Zod `dynamicImageTextPayloadSchema` + `sanitizeDynamicImageTextPayload` for zod errors; `sanitizeDynamicImageTextPayload` (rich-text-server) sanitizes each `content`/`contentJa` via `sanitizeRichTextHtml` (allowlist tags `RICH_TEXT_ALLOWED_TAGS`, styles `RICH_TEXT_ALLOWED_STYLES`). API routes `src/app/api/cms/page-sections/*` and `src/app/api/events/[id]/page-sections/*` call `parseSectionPayload` + `sanitizeDynamicImageTextPayload`.
- **Preview:** `src/components/admin/preview/ClientSectionPreview.tsx` — `sanitizeDraftSection` handles `items[].content/contentJa` draft sanitization, `DynamicPreview` renders sticky/static grid preview in Preview Studio.
- **Accessibility/Performance:** No scroll hijacking; keyboard navigable; alt text per image; respects reduced motion (sticky is layout, no animation); images use `next/image` with `sizes` and `unoptimized` for `/uploads/` (nginx serves directly). No per-frame JS.
- **Testing:** `npm run lint` baseline 81 errors/60 warnings unchanged; `npm run build` compiled successfully Static + Typed. Browser testing NOT PERFORMED (DATABASE_URL unavailable); Database testing NOT PERFORMED. Visual sticky behavior code-inspected only.
- **Files:** `prisma/schema.prisma`, `src/lib/page-section-types.ts`, `src/lib/page-section-payloads.ts`, `src/lib/section-layout.ts`, `src/lib/preview-layout-controls.ts`, `src/lib/rich-text-server.ts`, `src/components/content/sections/DynamicImageTextSection.tsx`, `src/components/content/sections/PageSectionRenderer.tsx`, `src/components/admin/PageSectionsManager.tsx`, `src/components/admin/preview/ClientSectionPreview.tsx`, `src/app/api/cms/page-sections/route.ts`, `src/app/api/cms/page-sections/[id]/route.ts`, `src/app/api/events/[id]/page-sections/route.ts`, `src/app/api/events/[id]/page-sections/[sectionId]/route.ts`.
