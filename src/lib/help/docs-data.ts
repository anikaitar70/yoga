...
export const helpSections: HelpSection[] = [
    ...
    body: [
      ...
      { type: "h2", text: "Toolbar buttons (RichTextEditor)" },
      {
        ...
        rows: [
          ...
          ["I", "Italic", "Ctrl+I"],
          ["U", "Underline", "Ctrl+U"],
          ["A + color", "Text color - colors the selected text itself (not the background). Select text first, then pick the color.", "-"],
    ...
    body: [
      ...
      { type: "p", text: "In every section editor, below the content fields, there is a collapsed Layout panel. For Program pages the simple presets (Spacing: Tight/Normal/Spacious and Content width: Narrow/Normal/Wide) are shown inline; fine-tuning sliders live in the Preview studio's Tune layout panel. Special event sections expose most sliders directly in the layout editor too." },
      {
        ...
        rows: [
          ...
          ["Heading horizontal position", "Preview studio -> Tune layout only (removed from the direct section editor, which now just links to it). Slider from −100 (heading's left edge at the container's left edge) to +100 (right edge at right edge), 0 = perfectly centered, step 1. It moves the heading with CSS transform so it never shifts layout or spills outside the container. Important: it only takes effect once you have explicitly touched the slider for that section - if you never open Tune layout for a section, publishing/saving never moves its heading."],
          ["Gap below heading", "−40 to 120px, step 1 - space between heading and subtitle/content. Collapses to a single margin (no more double-gap)."],
          ["Subheading colour", "Preview studio -> Tune layout. Color picker + hex input for the eyebrow/subtitle/tagline text in that section only (e.g. 'Creative life'). Leave empty to use the theme default; use Reset to clear an override."],
          ...
          ["Banner / image height", "HERO sections only, 120-640px, step 8. Now applies to the live public page too, not just the admin preview - the slider directly sets the rendered image container height."],
          ["Image aspect ratio", "HERO / IMAGE_TEXT / DYNAMIC_IMAGE_TEXT, 0.75-2.4, step 0.05. Fine control on top of the aspect presets below."],
          ["Image height / Aspect (presets)", "For IMAGE_TEXT sections: auto / landscape 16:9 / wide 21:9 / square / compact 4:3 presets."],
          ["Image fit", "Cover (crop to fill) or Contain (show whole image). Cover is best for photos; Contain is best for logos or images that must not be cropped."],
          ["Image position (focal point)", "Center / Top / Bottom / Left / Right - where Cover crops from when the image doesn't match the frame's aspect ratio."],
      ...
      { type: "note", variant: "info", text: "Program HERO sections used to silently ignore their top/bottom padding sliders - that has been fixed, so padding now actually applies there too." },
    ],
  },
  {
    id: "toc",
  ...
  },
  {
    ...
    body: [
      ...
      { type: "note", variant: "info", text: "Saving a BUTTON section on a Special Event with an empty label/link no longer errors out - it now falls back to a sensible default ('Book your retreat' / /contact) instead of a server error. Fill in your own label and URL if that default isn't what you want." },
    ],
  },
  {
    id: "translation",
  ...
  },
  {
    ...
    body: [
      ...
      { type: "h2", text: "Custom text block: Timeline & highlights" },
      { type: "p", text: "The Custom text block editor (also used inside Special events - see that section) can turn its body paragraphs into a numbered timeline and insert highlight/callout boxes between paragraphs." },
      { type: "note", variant: "warning", text: "When the timeline is switched on, the middle paragraphs (everything after the intro paragraph and before the closing paragraph) are NOT shown as plain text on the public page - they are replaced by the timeline rows below. Only the intro and closing paragraphs stay as text." },
      { type: "bullets", items: ["Linked mode - the timeline is generated automatically from your body paragraphs, numbered 01, 02, 03…", "Manual mode - label 'Manual - edit each timeline row below (numbers editable, e.g. \"1st\", \"02\", \"2021\")'. Each row has its own Number field (free text - e.g. '1st', '2021') and Text field, so you are not limited to sequential numbering.", "Import from paragraphs - copies your current body paragraphs into Manual rows so you can start editing them as a timeline.", "Push to paragraphs & link - the reverse: moves your Manual timeline rows back into the body paragraphs (kept between intro and closing) and switches back to Linked mode. Asks for confirmation before doing this.", "Highlight / callout boxes use an 'Insert after paragraph #' field. This number is 1-based as you type it - entering 1 inserts the highlight after the 1st paragraph."] },
    ],
  },
  {
    id: "special-events",
    ...
    body: [
      ...
      { type: "steps", items: ["Go to Special events -> click + New special event (or Create).", "Fill Title - the slug auto-derives from title; you can edit it manually. Slugs must be unique.", "Fill Summary (description), Location, Category, Starts/Ends dates, and the Special event hero image.", "Upload or choose an image via ImageUploadField (section=events). Alt text helps accessibility/SEO.", "Choose Published / Featured toggles.", "Fill SEO fields (title, description, og image, keywords) if desired.", "If you need a button on the dedicated page, fill Primary CTA / redirect: Button text + Button URL (https://… or /contact). Leave both blank to hide.", "Click Save general settings. The response validates title/location/dates/slug uniqueness and shows field errors inline if anything fails.", "Then scroll to Page content and add sections (Image + text items, Custom text, BUTTON, Gallery, etc.) - see Program pages for type details, including the Custom text block's Timeline & highlights behavior.", "Configure the Table of Contents (Automatic vs Custom) and select Testimonials for this event.", "Save each section (Save draft or Publish section). Use Preview (drafts) to check unpublished content and View live page for the public URL."] },
      { type: "h2", text: "Editing fields - what each means" },
      {
        ...
        rows: [
          ...
          ["Special event hero image", "Wide banner shown at the top of /events/special/[slug] (recommended ~21:9, 1600px wide). Separate from the card image - if left empty, the page falls back to the card image. Max 15MB, JPEG/PNG/WebP/GIF/SVG."],
  ...
  },
  {
    ...
    body: [
      ...
      { type: "note", variant: "info", text: "The Japanese (/ja/...) menu now always mirrors the English one item-for-item, including Testimonials, and the mobile 'More' grouping is worked out the same way regardless of locale - a /ja/gallery link groups under More exactly like /gallery does." },
  ...
  },
  {
    ...
    body: [
      ...
      { type: "p", text: "Each brand has a Logo scale slider 0.5x to 4x, step 0.05. It multiplies the logo size everywhere at once (navbar, footer, hero, admin sidebar). The slider label shows the resulting hero height in px (e.g. 'Logo scale (1.20x) - ~96px hero height'). The live preview card in the Branding editor shows all contexts at the chosen scale before you save. Logos upload via onLogoSave (saved immediately when upload finishes)." },
      { type: "h2", text: "Hero logo height (precise sizing)" },
      { type: "p", text: "In Design settings -> Header layout tab, below Logo scale, there is now a dedicated Hero logo height control (a 32-320px slider plus a 32-640px number input) for setting the homepage hero logo's pixel height precisely, separate from the scale used everywhere else. It is responsive and keeps the logo's aspect ratio inside its container. Click Reset to scale to remove the override and let the hero logo go back to being derived from Logo scale." },
  ...
  },
  {
    ...
    body: [
      ...
      { type: "bullets", items: ["Preview uses saved content (after you pressed Save/Publish). Unsaved edits visible only in the form you are still typing do NOT appear in the preview until you save.", "Preview includes fine-tuned layout sliders (section top/bottom padding, heading position, gap values) exactly as they appear live. Heading horizontal position is now only editable in Preview studio's Tune layout panel (it was removed from the direct section editor, which just links there instead).", "Preview does NOT use browser extensions or blocklists - it is the same Next.js render as the public page with an admin-only wrapper.", "A round of hydration/parity fixes removed a brief flash of default fonts/colors before design settings loaded and fixed several preview-vs-live mismatches (Hero and Image+Text rich text, Special Event previews) - Preview should now match the live page more reliably than before."] },
    ...
    body: [
      { type: "h2", text: "Common problems - what to check and try" },
      {
        ...
        items: [
          ...
          { q: "Heading has too much space above/below.", a: "Check Gap below heading and Section top/bottom padding sliders. Also check the Heading horizontal position slider in Preview studio -> Tune layout; it only affects horizontal transform, not height, and it only moves a heading once someone has explicitly touched it for that section." },
          { q: "A Hero heading moved after I published something unrelated.", a: "This bug is fixed - heading position now only changes when you explicitly set the Heading horizontal position slider in Preview -> Tune layout for that section. If you still see this, note the exact section and let your developer know." },
    ...
    body: [
      ...
      { type: "h2", text: "Actual deployment from this project (see DEPLOYMENT.md)" },
      {
        ...
        items: [
          ...
          "Before deploying a schema change: run npm run db:check-drift locally to confirm the Prisma schema and the committed migrations directory agree. Any model/field change must go through npx prisma migrate dev --name <description> to generate a real migration file - never rely on prisma db push reaching production.",
          "If package.json changed, rebuild: docker compose up -d --build. The entrypoint now runs prisma migrate deploy strictly, then migrate status plus the drift check, and warns with an actionable message instead of silently falling back to db push if something is wrong. It also consolidates SiteConfig and starts node server.js.",
          "Always after rebuilding the app: docker compose restart nginx (nginx caches the old app container IP).",
          "Verify: docker compose ps (app, db, nginx Up), curl -sS https://nirvanayoga.org/api/health should be {ok:true, db:true}, then run npm run deploy:smoke (or the equivalent curl checks) against /, /just-art-life, /events, and the /ja/ versions.",
          "Rollback if needed: git log --oneline -10, git reset --hard <good-commit>, docker compose up -d --build, docker compose restart nginx.",
        ],
      },
      { type: "note", variant: "warning", text: "/api/health is no longer a pure liveness probe - it now runs a real database query and returns 503 with a 'DB drift' hint if a column the code expects is missing (a P2022 error). A green {ok:true} means the database schema actually matches the code, not just that the server process is running." },
