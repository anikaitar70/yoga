...
import {
  DEFAULT_EVENTS_PAGE_SETTINGS,
  EVENTS_PAGE_INITIAL_COUNT_MAX,
  type EventsPageSettings,
} from "@/lib/events-page-settings";

interface EventManagerProps {
  initialEvents: AdminEvent[];
  initialPageSettings?: EventsPageSettings;
}
...
function normalizeAdminEvent(raw: Record<string, unknown>): AdminEvent {
  ...
  return {
    ...
    specialEventCtaLabel: raw.specialEventCtaLabel ? String(raw.specialEventCtaLabel) : null,
    specialEventCtaUrl: raw.specialEventCtaUrl ? String(raw.specialEventCtaUrl) : null,
  };
}
...
export default function EventManager({ initialEvents, initialPageSettings }: EventManagerProps) {
  ...
  const [pageSettings, setPageSettings] = useState<EventsPageSettings>(
    initialPageSettings ?? { ...DEFAULT_EVENTS_PAGE_SETTINGS },
  );
  ...
  const specialEvents = useMemo(
    () => sortedEvents.filter((event) => event.isSpecialEvent),
    [sortedEvents],
  );
  ...
  async function handleGroupReorder(groupEvents: AdminEvent[], index: number, direction: -1 | 1) {
    const target = index + direction;
    if (target < 0 || target >= groupEvents.length) return;
    const next = [...sortedEvents];
    const currentIndex = next.findIndex((event) => event.id === groupEvents[index].id);
    const swapIndex = next.findIndex((event) => event.id === groupEvents[target].id);
  ...
  async function savePageSettings() {
    ...
    setFeedback(null);
    try {
      const response = await adminFetch("/api/events/page-settings", {
        method: "PATCH",
        body: JSON.stringify(pageSettings),
      });
      const parsed = await parseAdminJsonResponse<EventsPageSettings>(response);
      if (!parsed.ok || !response.ok) {
        setFeedback(parsed.ok ? "Unable to save display settings." : parsed.error);
        return;
      }
      setPageSettings(parsed.data);
      setFeedback("Display settings saved.");
    } catch {
      setFeedback("Unable to save display settings.");
    } finally {
      setSettingsBusy(false);
    }
  }

  function renderEventListItem(eventItem: AdminEvent, index: number, groupEvents: AdminEvent[]) {
      ...
      <div key={eventItem.id} className="rounded-3xl border border-slate-200 bg-slate-50 p-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              onClick={() => handleGroupReorder(groupEvents, index, -1)}
            ...
            <button
              type="button"
              onClick={() => handleGroupReorder(groupEvents, index, 1)}
              disabled={index === groupEvents.length - 1 || reorderBusy}
              aria-label={`Move ${eventItem.title} down`}
              className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-700 disabled:opacity-40"
            >
              ↓
            </button>
            {eventItem.isSpecialEvent ? (
              <Link
                href={`/admin/special-events/${eventItem.id}`}
                className="rounded-2xl border border-violet-300 bg-violet-50 px-4 py-2 text-sm font-semibold text-violet-900 transition hover:bg-violet-100"
              >
                Sections
              </Link>
            ) : null}
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2 text-sm">
          ...
          <span
            className={`rounded-full px-2.5 py-1 ${
              eventItem.isSpecialEvent ? "bg-violet-100 text-violet-900" : "bg-slate-100 text-slate-600"
            }`}
          >
            {eventItem.isSpecialEvent ? "Special Event" : "Regular Class"}
          </span>
          <span
            className={`rounded-full px-2.5 py-1 ${
              status.hasExternal ? "bg-emerald-100 text-emerald-800" : "bg-slate-100 text-slate-500"
            }`}
          >
          ...
          <span
            className={`rounded-full px-2.5 py-1 ${
              status.hasReadMore ? "bg-amber-100 text-amber-900" : "bg-slate-100 text-slate-500"
            }`}
          >
            {status.hasReadMore
          ...
          {status.hasRegistration ? (
            <span className="rounded-full bg-sky-100 px-2.5 py-1 text-sky-900">Google Form CTA</span>
          ) : null}
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-600">
              Order events, classify special events vs regular classes, and configure how many appear before Expand on the public Events page.
            </p>
          </div>
        ...
        <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-5">
          <h4 className="text-sm font-semibold text-slate-900">Public page display limits</h4>
          <p className="mt-1 text-xs text-slate-500">
            Number of items shown initially in each section before the Expand button.
          </p>
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <label className="block text-sm font-medium text-slate-700">
              Special Events — initial shown
              <input
                type="number"
                min={1}
                max={EVENTS_PAGE_INITIAL_COUNT_MAX}
                value={pageSettings.specialEventsInitialCount}
                onChange={(event) =>
                  setPageSettings((current) => ({
                    ...current,
                    specialEventsInitialCount: Number(event.target.value),
                  }))
                }
                className="mt-2 w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 outline-none"
              />
            ...
            <label className="block text-sm font-medium text-slate-700">
              Regular Classes — initial shown
              <input
                type="number"
                min={1}
                max={EVENTS_PAGE_INITIAL_COUNT_MAX}
                value={pageSettings.regularClassesInitialCount}
                onChange={(event) =>
                  setPageSettings((current) => ({
                    ...current,
                    regularClassesInitialCount: Number(event.target.value),
                  }))
                }
                className="mt-2 w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 outline-none"
              />
          ...
          <button
            ...
            className="mt-4 rounded-2xl bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-700 disabled:opacity-60"
          >
            {settingsBusy ? "Saving…" : "Save display settings"}
          </button>
        </div>

        <div className="mt-8 space-y-8">
          <div className="space-y-4">
            <h4 className="text-sm font-semibold uppercase tracking-wide text-violet-900">Special Events</h4>
            {specialEvents.length > 0 ? (
              specialEvents.map((eventItem, index) => renderEventListItem(eventItem, index, specialEvents))
            ) : (
              <p className="text-sm text-slate-600">No special events yet.</p>
            )}
          </div>

          <div className="space-y-4">
            <h4 className="text-sm font-semibold uppercase tracking-wide text-slate-700">Regular Classes</h4>
            {regularClasses.length > 0 ? (
              regularClasses.map((eventItem, index) => renderEventListItem(eventItem, index, regularClasses))
            ) : (
              <p className="text-sm text-slate-600">No regular classes yet.</p>
            )}
          </div>
        </div>
      </section>
