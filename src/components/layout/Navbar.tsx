    ...
    >
      {nav.map((item) => (
        <Link
          ...
          className={cn(
            "relative rounded-md px-3.5 py-2 transition-colors duration-300",
          )}
        >
      ...
      >
        {headerLayout.alignment === "center" ? (
          <>
            ...
            <div className="flex justify-center">{logoLink}</div>
        ...
        <Container className="flex flex-col gap-1 py-5">
          {nav.map((item) => (
            <Link
              ...
              aria-current={pathname === item.href ? "page" : undefined}
              className={cn(
                "rounded-md px-4 py-3.5 transition-colors hover:bg-surface-warm",
              )}
            >
              {isJustArtNavItem(item) ? (
                <BrandLogo
