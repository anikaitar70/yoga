...
export function EventDetailPanel({
  ...
  const show = open && Boolean(detail) && (preview || eventDetailHasReadableContent(detail));
  ...
  const registration =
    detail?.registration?.enabled && detail.registration.googleFormUrl
      ? detail.registration
      : null;
    ...
    <AnimatePresence>
      {show && detail ? (
        ...
        >
          <button
            type="button"
            aria-label={uiMessage(locale, "closeEventDetail", localeContent)}
            ...
            <div className="flex items-start justify-between gap-4 border-b border-border/60 px-5 py-4 sm:px-8 sm:py-5">
              <div className="min-w-0 space-y-1.5">
                ...
                <p id={descriptionId} className="text-sm text-muted">
                  {formatEventRange(event.date, event.endDate, locale)}
